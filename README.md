...
Mô phỏng giao diện Dashboard thay đổi theo 8 cấp độ quyền hạn (Từ Superuser, Global Admin đến các Org Roles).
...
2. backend/: **Đây CHỈ LÀ MOCK DATA (Dữ liệu giả lập).** Thư mục này chứa một server Node.js nhỏ giọt được viết ra trong giai đoạn đầu để test giao diện (trước khi có CVAT thật). **KHÔNG CẦN chạy thư mục này** nếu đã có CVAT thật chạy trên máy.
...
**Bước 4: Tạo tài khoản Superuser**
...
*(Lưu ý: Hệ thống Vite đã được cấu hình sẵn hệ thống Proxy ngầm trong file vite.config.js để tự động đẩy mọi API sang http://localhost:8080 và lách luật CORS của trình duyệt, không cần cấu hình gì thêm).*
